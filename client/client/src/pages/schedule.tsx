...
import { Plus, ChevronLeft, ChevronRight, Clock, MapPin, X, Check, Ban, Trash2, UserX } from "lucide-react";
import {
  format,
  startOfMonth,
  endOfMonth,
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    {session.status === "scheduled" && (
                      <>
                        ...
                        <Button
                          ...
                          size="icon"
                          className="w-7 h-7"
                          onClick={() => updateStatus({ id: session.id, status: "no_show" })}
                          data-testid={`button-no-show-${session.id}`}
                    ...
                    {session.status === "no_show" && (
                      <Badge variant="secondary" className="text-xs bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-400">
                        No-show
                      </Badge>
                    )}
                  </div>
